...
_Z(prini,Pv(ax,prini(en(x),f))Pv(tx!=2,print(x);xx())i(nx,fputc(((i2*)sx)[i],f));_r(x),Ux,FILE*f)_Z(printi,prini(x,stdout);O("\n"),Ux)
f(t2s,P(tx,x)Ur=ti('(');i(nx,r=cat(r,t2s(at(rx,i))))r=cat(r,ti(')'));r(r,_r(x)))
/*parse*/Z0*m[]={"av;","['/\\",":+-*%&|<>=~.!@?#_^,$LMS...ERZ",";)]\n "},*P,c[128];_U(fi,sd=s-1;W(*++d)P(*d==ix,ti(d-s));0,ss,Ux)f(v,fi(P,x))
static i2 i,*sb,nb;_D(i2,s,P(i>=nb,' ')sb[i],ii)
...
_Z(Ax,$(!ix,O("err"));,Ux)F(mat,P(ax,ti(ix==ia))P(ta!=tx||na!=nx,ti(0))i(nx,P(!mat(at(rx,i),at(ra,i)),ti(0)))ti(1))
Z0*pts[][2]={{"0","0"},{"1+2","+12"},{"+/x","(/+)x"},{0}};_Z(pt,i2 i=0;W(*pts[i])(Ax(mat(t2s(parse(pts[i][0])),p_(pts[i][1]))),i++))
int main(int n,i0**v){ini();
 Ux=t2s(parse("0"));
 printi(x);
 printi(t2s(parse("1+2")));
 printi(eg(parse("0")));
 return 0;
}
