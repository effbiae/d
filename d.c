...
_U(t,P(q(),el)Ux=!fi("([",s(i))?n():(x=E(ti(s(i))),3>nx)?_(A(nx>1);at(x,1)):x;W('a'==c[(i0)s(i)])x='['==s(i)?E(x):_2(n(),x);x)
...
f(eg,
     ...
     )P(train(ra),U t=_2(tra(a),eg(at(rx,1)));nx==2?_x(pc("%(%)",t)):pc("%(%,%)",cat(t,en(eg(at(x,2))))))
     0
     )
