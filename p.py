...
def s0(i):return s[i] if type(s[i])==type(b' '[0]) else s[i][0]
def cs(i):return c[s0(i)]
def n():
 global i
 if s[i]:i+=1;return s[i-1]
 return 0;
def q():return b';'[0]==cs(i)
def E(x):
 while n() in b";[(":
...
def t():
 if q():return []
 x=n() if b'('[0]-s0(i) else x[1] if 3>len(x:=E(s[i])) else x
 while b'a'[0]==cs(i):
  x=E(x) if b'['[0]==s0(i)else[n(),x]
 return x
def v():return b';'[0]<cs(i-1)
def e(x):
 if q():return x
...
def lex(x):
 r=[];i=0
 while i<len(x):
  if not x[i]or x[i+1]-b':'[0]:r.append(x[i])
